...
public class ItemService {
	...
	@Transactional
	public List<ItemListResponse> updateStatusChangeTime(final Project project,
		final List<Item> itemListResponses) {
		...
		try {
			...
			mostLastSuccessTime = itemListResponses.get(0).getLastSuccessDate();
			mostLastFailureTime = itemListResponses.get(0).getLastFailureDate();

			for (Item item : itemListResponses) {
				String status;
				...
				final Duration duration = Duration.between(failureDate, successDate);
				// 초 단위 차이
				...
				if (diffInSeconds >= 0) {
					...
					if (successDate.isAfter(mostLastFailureTime)) {
						mostLastFailureTime = failureDate;
					}
				}
		...
		final Project project1 = projectService.modifyProject(mostLastSuccessTime, mostLastFailureTime, project);
		System.out.println("성공, 실패 값: " + mostLastSuccessTime + " / " + mostLastFailureTime);
		System.out.println("프로젝트 반영 값"+project1.getLastSuccessDate()+" / "+project1.getLastFailureDate());
